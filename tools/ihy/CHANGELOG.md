...
## v1.0.3 — visual and functional parity rebuild (20 Jun 2026)

This rebuild repackages the last working Ihy standalone editor (v0.56) into a `tools/ihy/` folder for integration with a new project structure.  All original source files have been retained to avoid breaking implicit dependencies.  No new functionality has been added, and no existing functionality has been intentionally removed.  The aim was to preserve behaviour and prepare for future refactoring.  Note that this rebuild does not yet match the visual style of the v0.41 screenshot supplied during this exercise; further work is required to reconcile those differences.
