 ...
 * v0.23 — fresh module build; parent WINDOW capture for Explorer drops; source-pixel peak strips.
 * Basic Mode is untouched.
 */

import { AdvancedTimeline } from './advanced-timeline-v0.23.js';
import { AdvancedMediaEngine } from './advanced-media-engine-v0.23.js';
...
(() => {
    ...
    function installGlobalFileDrop() {
        ...
        try {
            ...
            if (parentDocument?.body) {
                ...
                // Window capture runs before Document capture. This matters because the
                // Organon Hub may have its own Document-level drag handler which otherwise
                // receives the Explorer drop first and prevents the Advanced page from seeing it.
                parentWindow.addEventListener('dragenter', showBridge, true);
                parentWindow.addEventListener('dragover', showBridge, true);
                parentWindow.addEventListener('dragleave', scheduleBridgeHide, true);
                parentWindow.addEventListener('drop', receiveParentFiles, true);
                ...
                cleanupParentBridge = () => {
                    parentWindow.removeEventListener('dragenter', showBridge, true);
                    parentWindow.removeEventListener('dragover', showBridge, true);
                    parentWindow.removeEventListener('dragleave', scheduleBridgeHide, true);
                    parentWindow.removeEventListener('drop', receiveParentFiles, true);
    ...
    requestAnimationFrame(() => showToast('Advanced Audio Timeline v0.23 FRESH loaded — drop new files anywhere, then drag Project Files onto the timeline'));
})();
