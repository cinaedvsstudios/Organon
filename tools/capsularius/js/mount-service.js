  ...
  async function mountDirectory(handle, { targetWindow = null, source = 'picker' } = {}) {
    ...
    for (const existing of state.mounts.values()) {
      if (await handlesAreSame(existing.handle, handle)) {
        ...
        existing.health = existing.permission === 'granted' ? 'connected' : 'permission-required';
        existing.healthDetail = existing.permission === 'granted' ? 'Folder opens normally.' : 'Browser permission is needed to reopen this folder.';
    ...
    mount.health = mount.permission === 'granted' ? 'connected' : 'permission-required';
    mount.healthDetail = mount.permission === 'granted' ? 'Folder opens normally.' : 'Browser permission is needed to reopen this folder.';
