...
const icon = { library:'📚', recents:'🕘', drive:'☁️', folder:'📁', archive:'🗜️', reconnect:'🔗' };
...
function isZipFile(entry) { return entry?.kind === 'file' && extensionOf(entry.name) === 'zip'; }
...
export function installTree(Workspace) {
  ...
  Workspace.prototype.treeChildren=async function treeChildren(record,source){
    ...
    try {
      ...
      if (source.kind === 'google-drive') {
        ...
        children=(await this.archiveService.listSource(source))
          .filter((entry)=>entry.kind === 'directory')
          .map((entry)=>({
            name:entry.name,
            source:this.archiveService.childSource(source,entry),
            colour:this.state.mounts.get(source.mountId)?.colour,
            icon:icon.folder
          }));
      } else {
        ...
        children=result.entries
          .filter((entry)=>entry.kind === 'directory' || isZipFile(entry))
          .map((entry)=>{
            if (entry.kind === 'directory') return { name:entry.name,source:physicalSource(source.mountId,[...source.pathSegments,entry.name]),colour:mount.colour,icon:icon.folder };
            return { name:entry.name,source:zipSource(source.mountId,source.pathSegments,entry.name),colour:mount.colour,icon:icon.archive,subtitle:'ZIP archive' };
          })
          .sort(sortName);
      }
      ...
      this.onToast(source.kind === 'zip' ? 'Capsularius could not read this ZIP archive.' : 'Capsularius could not read child folders.','error');
      return [];
    } finally {
  ...
  Workspace.prototype.expandTreeForSource=async function expandTreeForSource(record,source){
    ...
    if (source.kind === 'zip') {
      const physicalParent=physicalSource(source.mountId,source.parentPathSegments);
      record.treeExpanded.add(sourceKey(physicalParent));
      await this.treeChildren(record,physicalParent);
      const pieces=String(source.zipPath || '').replace(/\/$/,'').split('/').filter(Boolean);
      for (let depth=0;depth<pieces.length;depth+=1) {
        const parent=zipSource(source.mountId,source.parentPathSegments,source.archiveName,depth ? `${pieces.slice(0,depth).join('/')}/` : '');
        record.treeExpanded.add(sourceKey(parent));
        await this.treeChildren(record,parent);
      }
    }
  ...
  Workspace.prototype.renderTreeNode=function renderTreeNode(record,source,label,colour,depth,options={}){
    ...
    if (options.noExpander || (empty && !loading)) {
      expander.disabled=true;
      expander.classList.add('empty');
      expander.textContent='';
    } else {
      expander.addEventListener('click',(event)=>{ event.stopPropagation();this.toggleTree(record,source); });
    }
    const glyph=el('span','tree-folder-icon',options.icon || icon.folder);
    glyph.style.color=colour || 'var(--stone)';
    const button=el('button','tree-label',label);
    button.type='button';
    ...
    if (options.reconnect) {
      const reconnect=el('button','tree-access-state',icon.reconnect);
      reconnect.type='button';reconnect.title='Reconnect';
  ...
  Workspace.prototype.renderSidebar=function renderSidebar(record){
    ...
    if (!sidebar) return base.sidebar.call(this,record);
  ...
  Workspace.prototype.loadWindow=async function loadWindow(record,...args){
    ...
    const value=await base.load.call(this,record,...args);
    if (record.source.kind === 'physical' || record.source.kind === 'zip') this.expandTreeForSource(record,record.source).catch(console.error);
    else this.renderSidebar(record);
    return value;
  };
  Workspace.prototype.navigateWindow=async function navigateWindow(record,source,options={}){
    const value=await base.navigate.call(this,record,source,options);
    if (source.kind === 'physical' || source.kind === 'zip') await this.expandTreeForSource(record,source);
    this.renderSidebar(record);
    return value;
  };
  Workspace.prototype.renderBreadcrumbs=function renderBreadcrumbs(record){
    if (record.source.kind !== 'google-drive') return base.breadcrumbs.call(this,record);
    const holder=record.element.querySelector('.breadcrumbs');holder.replaceChildren();
    crumbs(record.source).forEach((source,index,chain)=>{
