...
function listState(record) {
  if (!record.listState) {
    record.listState = { sortKey:'name', sortDirection:'asc', filters:{ name:'', types:[], extensions:[], size:'any', modified:'any', created:'any' } };
  }
...
function openProperties(workspace, record, entries) {
  document.querySelector('.caps-properties-panel')?.remove();
  if (!entries.length) return;
  const single = entries.length === 1;
  const entry = entries[0];
  const panel = el('section', 'caps-properties-panel');
  const drag = el('div', 'caps-panel-drag', 'Capsularius · Properties');
  panel.append(drag, el('h2', '', single ? entry.name : `${entries.length} selected items`));
  const details = el('dl', 'caps-properties-grid');
  const add = (label, value) => details.append(el('dt', '', label), el('dd', '', value));
  if (single) {
    add('Location', sourcePathLabel(workspace.state, record.source));
    ...
    add('Created', formatDate(createdFor(entry)));
    add('Length / dimensions', infoFor(entry));
  } else {
  ...
  const actions = el('div', 'caps-panel-actions');
  if (single) {
    ...
    save.addEventListener('click', () => {
      const next = {}; fields.querySelectorAll('[data-property]').forEach((input) => { next[input.dataset.property] = input.value.trim(); });
      metadata[propertyId(workspace, record, entry)] = next;
      writeJson(PROPERTY_KEY, metadata);
      workspace.onToast('Properties saved in Capsularius.', 'success');
    });
    actions.append(save);
  }
  const close = el('button', '', 'Close'); close.addEventListener('click', () => panel.remove()); actions.append(close);
  panel.append(actions); document.body.append(panel);
...
function showFilter(workspace, record, key, event) {
  event.preventDefault(); closeFilter();
  ...
  if (key === 'name') {
    const input = document.createElement('input'); input.type = 'search'; input.value = state.filters.name; input.placeholder = 'Name contains…'; input.dataset.nameFilter = 'true'; body.append(input);
  } else if (key === 'type') {
    ...
    types.forEach((type) => {
      const label = el('label', 'caps-filter-check'); const input = document.createElement('input'); input.type='checkbox'; input.value=type; input.checked=state.filters.types.includes(type); label.append(input, el('span','',type)); body.append(label);
    });
    if (extensions.length) {
      body.append(el('strong','caps-filter-group-title','Extensions present here'));
      extensions.forEach((extension) => {
        const label = el('label','caps-filter-check'); const input = document.createElement('input'); input.type='checkbox'; input.value=`.${extension}`; input.checked=state.filters.extensions.includes(extension); label.append(input,el('span','',`.${extension}`)); body.append(label);
      });
    }
  } else {
    body.append(el('div','','Use a column click to sort. Type and Name support filters.'));
  }
  const actions = el('div', 'caps-filter-actions');
  const clear = el('button','','Clear'); const apply = el('button','primary','Apply');
  clear.addEventListener('click', () => {
    if (key === 'name') state.filters.name = '';
    if (key === 'type') { state.filters.types=[]; state.filters.extensions=[]; }
  ...
  apply.addEventListener('click', () => {
    ...
    if (key === 'type') {
      ...
      state.filters.types = checked.filter((value) => !value.startsWith('.'));
      state.filters.extensions = checked.filter((value) => value.startsWith('.')).map((value) => value.slice(1));
    }
    closeFilter(); workspace.renderWindow(record);
  });
  ...
  COLUMNS.forEach(([key,label], index) => {
    ...
    if (index < COLUMNS.length - 1) {
      ...
      resize.addEventListener('pointerdown',(event) => {
        if (event.button !== 0) return;
        event.preventDefault(); const all=columnWidths(); const startX=event.clientX; const initial=all[key];
        resize.classList.add('dragging'); resize.setPointerCapture(event.pointerId);
        const move=(pointer) => { all[key]=Math.max(64,Math.min(560,Math.round(initial + pointer.clientX - startX))); writeJson(COLUMN_KEY,all); document.querySelectorAll('.window-content').forEach((pane)=>pane.style.setProperty('--caps-columns',columnTemplate())); };
...
function setupSidebarResize(record) {
  ...
  divider.addEventListener('pointerdown',(event) => {
    if (event.button !== 0) return; event.preventDefault(); const startX=event.clientX; const startWidth=Number.parseFloat(getComputedStyle(main).getPropertyValue('--caps-sidebar-width')) || saved;
    divider.classList.add('dragging'); divider.setPointerCapture(event.pointerId);
...
function scheduleMetadata(workspace, record, token) {
  ...
  const step=async()=>{
    ...
    const slice=pending.slice(cursor,cursor+8); cursor+=slice.length; await Promise.allSettled(slice.map(hydrateFileEntry));
    if (record.uiToken !== token || !record.element?.isConnected) return;
    workspace.renderWindow(record);
...
export function installWorkspaceUi(Workspace) {
  ...
  Workspace.prototype.renderItem=function renderItem(record,entry,index,visible){const node=base.renderItem.call(this,record,entry,index,visible);if(record.viewMode!=='list')return node;node.classList.add('caps-list-row');node.style.setProperty('--caps-columns',columnTemplate());const type=fileTypeDescriptor(entry.name);const size=node.querySelector('.file-meta');if(size)size.textContent=formatBytes(entry.size);node.append(el('span','caps-list-cell modified',formatDate(entry.lastModified)),el('span','caps-list-cell type',entry.kind==='directory'?'Folder':entry.kind==='shortcut'?'Location':type.label),el('span','caps-list-cell info',infoFor(entry)),el('span','caps-list-cell created',formatDate(createdFor(entry))));const icon=node.querySelector('.file-icon');if(icon&&entry.kind==='file'&&!icon.querySelector('img'))icon.textContent=type.icon;return node;};
  ...
  Workspace.prototype.showContextMenu=function showContextMenu(event,record,entry){base.showContextMenu.call(this,event,record,entry);const menu=this.contextMenu;if(!menu||menu.hidden)return;if(!menu.querySelector('[data-command="refresh"]')){const refresh=el('button','context-item','🔄 Refresh');refresh.type='button';refresh.dataset.command='refresh';menu.insertBefore(refresh,menu.firstChild);}if(entry&&!menu.querySelector('[data-command="properties"]')){const prop=el('button','context-item','ℹ️ Properties…');prop.type='button';prop.dataset.command='properties';const before=[...menu.querySelectorAll('button')].find((button)=>button.dataset.command==='delete')||null;menu.insertBefore(prop,before);}};
  const baseBind=Workspace.prototype.bindContextMenu;
  Workspace.prototype.bindContextMenu=function bindContextMenu(){baseBind.call(this);if(this.contextMenu.dataset.workspaceUiBound)return;this.contextMenu.dataset.workspaceUiBound='true';this.contextMenu.addEventListener('click',(event)=>{const command=event.target.closest('button')?.dataset.command;if(command!=='properties')return;event.preventDefault();event.stopImmediatePropagation();const record=this.state.windows.get(Number(this.contextMenu.dataset.windowId));const entry=record?.items.find((item)=>item.id===this.contextMenu.dataset.entryId);this.hideContextMenu();if(record&&entry)openProperties(this,record,this.getSelectedEntries(record));},true);};
  Workspace.prototype.tileVisibleWindows=function tileVisibleWindows(){const visible=[...this.state.windows.values()].filter((record)=>!record.minimized);if(!visible.length)return;const margin=14,gap=12,width=this.viewport.clientWidth,height=this.viewport.clientHeight;const rows=visible.length<=3?[visible.length]:Array.from({length:Math.ceil(visible.length/Math.ceil(Math.sqrt(visible.length)))},(_,row)=>Math.min(Math.ceil(Math.sqrt(visible.length)),visible.length-row*Math.ceil(Math.sqrt(visible.length))));const usableHeight=height-margin*2-gap*(rows.length-1);let cursor=0,y=margin;this.state.workspace.panX=0;this.state.workspace.panY=0;this.applyWorkspaceTransform();for(const count of rows){const cellW=(width-margin*2-gap*(count-1))/count;const cellH=usableHeight/rows.length;let x=margin;for(let index=0;index<count;index+=1){const record=visible[cursor++];record.x=Math.round(x);record.y=Math.round(y);record.width=Math.floor(index===count-1?width-margin-x:cellW);record.height=Math.floor(cellH);Object.assign(record.element.style,{left:`${record.x}px`,top:`${record.y}px`,width:`${record.width}px`,height:`${record.height}px`});x+=cellW+gap;}y+=cellH+gap;}this.onStateChange();};
}
