...
function bringToFront(panel) {
  const highest = Math.max(14000,...[...document.querySelectorAll('.caps-asset-window')].map((item)=>Number(item.style.zIndex || 14000)));
...
async function addPreviewContent(panel, entry) {
  ...
  if(entry.fileType==='audio'){
    const audioBox=el('div','caps-preview-audio');audioBox.append(el('span','caps-preview-audio-icon','🎵'),el('strong','',entry.name));const audio=document.createElement('audio');audio.controls=true;audio.preload='metadata';audio.src=url;audioBox.append(audio);content.append(audioBox);return;
  }
  if(PDF_EXTENSIONS.has(extension)){
    const frame=document.createElement('iframe');frame.className='caps-preview-pdf';frame.title=entry.name;frame.src=url;content.append(frame);return;
  }
  if(TEXT_EXTENSIONS.has(extension)||entry.mimeType.startsWith('text/')){
    const text=await file.text();const pre=el('pre','caps-preview-text');pre.textContent=text.length>250000?`${text.slice(0,250000)}\n\n— Preview limited to the first 250,000 characters —`:text;content.append(pre);return;
  }
  unsupportedPreview(content,entry);
}

export async function openPreviewWindow(workspace, record, entry) {
  if(!entry?.handle||entry.kind!=='file'){workspace.onToast('Preview is available for one local file at a time.','error');return;}
  const type=fileTypeDescriptor(entry.name);const panel=createWindow({kind:'preview',title:entry.name,subtitle:type.label});panel.append(el('div','caps-preview-loading','Loading preview…'));positionPanel(panel,24);
  try{await hydrateFileEntry(entry);panel.querySelector('.caps-preview-loading')?.remove();await addPreviewContent(panel,entry);}catch(error){panel.querySelector('.caps-preview-loading')?.remove();const failed=el('div','caps-preview-empty');failed.append(el('span','caps-preview-empty-icon',type.icon || '📄'),el('strong','',entry.name),el('span','',error?.message || 'This file could not be previewed.'));panel.append(failed);}
}

export async function openPropertiesWindow(workspace, record, entries) {
  if(!entries.length)return;
  const single=entries.length===1;const entry=entries[0];
  if(single&&entry?.handle){try{await hydrateFileEntry(entry);}catch(_){/* show the metadata available */}}
  ...
  if(single){
    const metadata=readJson(PROPERTY_KEY,{});const saved=metadata[propertyId(workspace,record,entry)] || {};const fields=el('div','caps-properties-fields');
