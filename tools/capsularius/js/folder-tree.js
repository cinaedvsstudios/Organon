import { physicalSource, sourceKey } from './state.js';
...
export function installFolderTree(Workspace) {
  ...
  Workspace.prototype.renderSidebar = function renderTreeSidebar(windowRecord) {
    ...
    for (const mount of this.state.mounts.values()) {
      ...
      mounted.append(this.renderTreeNode(windowRecord, rootSource, mount.nickname || mount.name, mount.colour, 0));
    }
  };

  Workspace.prototype.renderTreeNode = function renderTreeNode(windowRecord, source, label, colour, depth) {
    const key = sourceKey(source);
    const expanded = windowRecord.treeExpanded.has(key);
    const loading = windowRecord.treeLoading.has(key);
    const children = windowRecord.treeChildren.get(key);
    const active = sameSource(windowRecord.source, source);

    const wrapper = makeElement('div', 'tree-node-wrap');
    const row = makeElement('div', `tree-node${active ? ' active' : ''}`);
    ...
    if (children && children.length === 0 && !loading) {
    ...
    const icon = makeElement('span', 'tree-folder-icon', '▰');
    ...
    if (expanded) {
      if (loading) {
        ...
        for (const child of children) {
          wrapper.append(this.renderTreeNode(windowRecord, child.source, child.name, colour, depth + 1));
        }
      }
    }
    return wrapper;
