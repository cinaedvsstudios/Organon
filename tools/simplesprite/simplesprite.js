(() => {
  ...
  function setCanvasDimensions(width, height, { resetMask = true } = {}) {
    ...
    elements.overlayCanvas.width = safeWidth;
    elements.overlayCanvas.height = safeHeight;
  ...
  function overlaySprite() {
    return state.sprites.find((sprite) => sprite.id === state.overlaySpriteId) || null;
  }

  function renderOverlaySprite() {
    ...
    overlayContext.drawImage(sprite.image, 0, 0, elements.overlayCanvas.width, elements.overlayCanvas.height);
    overlayContext.restore();
  }

  function hideEraserCursor() {
    state.cursorVisible = false;
    elements.eraserCursor.classList.remove('is-visible');
  }

  function updateEraserCursor(point) {
    if (!state.eraserEnabled || !state.image || !point) {
      hideEraserCursor();
      return;
    }
    const displayScale = elements.canvasFrame.clientWidth / elements.spriteCanvas.width;
    const displayBrush = Math.max(2, state.eraserSize * displayScale);
    elements.eraserCursor.style.width = `${displayBrush}px`;
    elements.eraserCursor.style.height = `${displayBrush}px`;
    elements.eraserCursor.style.marginLeft = `${-displayBrush / 2}px`;
    elements.eraserCursor.style.marginTop = `${-displayBrush / 2}px`;
    elements.eraserCursor.style.left = `${point.x * displayScale}px`;
    elements.eraserCursor.style.top = `${point.y * displayScale}px`;
    state.cursorVisible = true;
    elements.eraserCursor.classList.add('is-visible');
  }

  function toggleSpriteOverlay(spriteId) {
    state.overlaySpriteId = state.overlaySpriteId === spriteId ? null : spriteId;
    renderOverlaySprite();
    renderSprites();
    if (state.overlaySpriteId) setStatus('Sprite overlay enabled at 50% transparency.', 'success');
    else setStatus('Sprite overlay cleared.');
  }
  ...
  function fitCanvasPreview() {
    ...
    if (!state.cursorVisible) hideEraserCursor();
  }
  ...
  function updateEraserToggle() {
    ...
    if (!state.eraserEnabled) hideEraserCursor();
    renderAll();
  }
  ...
  function renderSprites() {
    ...
    state.sprites.forEach((sprite, index) => {
      ...
      scale.className = 'sprite-scale';
      scale.textContent = `Scale ${sprite.zoomPercent}%`;
      copy.append(number, dimensions, scale);
      const actions = document.createElement('div');
      actions.className = 'sprite-actions';
      const overlay = document.createElement('button');
      overlay.className = `overlay-sprite${state.overlaySpriteId === sprite.id ? ' is-active' : ''}`;
      overlay.type = 'button';
      overlay.textContent = '👁';
      overlay.title = state.overlaySpriteId === sprite.id ? 'Hide alignment overlay' : 'Show alignment overlay';
      overlay.setAttribute('aria-label', overlay.title);
      overlay.addEventListener('click', () => toggleSpriteOverlay(sprite.id));
  ...
  function removeSprite(spriteId) {
    ...
    if (index === -1) return;
    if (state.overlaySpriteId === spriteId) {
      state.overlaySpriteId = null;
      renderOverlaySprite();
    }
  ...
  function bindEvents() {
    ...
    elements.eraserSize.addEventListener('input', () => {
      ...
      updateRangeLabels();
      if (state.cursorVisible) updateEraserCursor({ x: state.pointer.lastX || elements.spriteCanvas.width / 2, y: state.pointer.lastY || elements.spriteCanvas.height / 2 });
    });
    ...
    elements.spriteCanvas.addEventListener('pointerleave', () => hideEraserCursor());
    elements.spriteCanvas.addEventListener('pointerenter', (event) => { if (state.image && state.eraserEnabled) updateEraserCursor(getCanvasPoint(event)); });
