...
(function installOrgavoxMarkers() {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .orgavox-marker-list-row{display:grid!important;grid-template-columns:minmax(130px,1.2fr) 112px 112px auto auto!important;align-items:center!important;gap:8px!important;width:100%!important;padding:8px 10px!important;border:1px solid rgba(224,163,96,.34)!important;border-radius:12px!important;background:rgba(0,0,0,.28)!important;color:#f5f0db!important;text-align:left!important}
      .orgavox-marker-list-row input,.orgavox-marker-list-row select{height:32px!important;border:1px solid rgba(117,178,222,.45)!important;border-radius:9px!important;background:#050505!important;color:#f5f0db!important;padding:0 8px!important;font:800 .74rem var(--font-mono,monospace)!important;min-width:0!important}
      .orgavox-marker-list-row .danger{border-color:rgba(220,72,64,.72)!important;color:#ffd8d2!important}
      .orgavox-marker-list-row .tool-button{min-height:32px!important;padding:6px 9px!important;font-size:.66rem!important}
      .orgavox-marker-help{padding:8px 10px!important;border:1px solid rgba(117,178,222,.28)!important;border-radius:11px!important;background:rgba(117,178,222,.07)!important;color:rgba(245,240,219,.72)!important;font-size:.72rem!important}
      @media(max-width:760px){.orgavox-marker-list-row{grid-template-columns:1fr!important}}
    `;
  ...
  function updateMarker(id, updates = {}) { const marker = markers().find((item) => item.id === id); if (!marker) return; marker.label = String(updates.label ?? marker.label ?? "Marker").trim() || "Marker"; marker.time = parseTimeValue(updates.time ?? marker.time, marker.time); marker.color = COLORS.includes(updates.color) ? updates.color : marker.color || COLORS[0]; closeContext(); forceRenderMarkers(); window.orgavoxRecordHistory?.(); showToast("Marker updated."); }
  function focusMarkerRow(id) { const modal = ensureModal(); modal.hidden = false; renderList(); const row = modal.querySelector(`[data-marker-row="${id}"]`); row?.querySelector("[data-marker-label]")?.focus(); row?.scrollIntoView?.({ block: "center" }); }
  function saveMarkerRow(row) { if (!row) return; updateMarker(row.dataset.markerRow, { label: row.querySelector("[data-marker-label]")?.value, time: row.querySelector("[data-marker-time]")?.value, color: row.querySelector("[data-marker-color]")?.value }); }
  function cycleMarkerColor(id) { const marker = markers().find((item) => item.id === id); if (!marker) return; const index = COLORS.indexOf(marker.color); updateMarker(id, { color: COLORS[(index + 1) % COLORS.length] }); }
  function openMarkerContext(id, x, y) { const marker = markers().find((item) => item.id === id); if (!marker) return; const menu = ensureContext(); menu.innerHTML = `<button class="tool-button" data-action="edit" type="button">Edit Marker…</button><button class="tool-button" data-action="jump" type="button">Jump to Marker</button><button class="tool-button" data-action="move-playhead" type="button">Move Marker to Playhead</button><button class="tool-button" data-action="color" type="button">Cycle Color</button><button class="tool-button danger" data-action="delete" type="button">Delete Marker</button>`; menu.querySelector("[data-action='edit']")?.addEventListener("click", () => { closeContext(); focusMarkerRow(id); }); menu.querySelector("[data-action='jump']")?.addEventListener("click", () => { closeContext(); jumpToMarker(id); }); menu.querySelector("[data-action='move-playhead']")?.addEventListener("click", () => moveMarker(id, state.playhead)); menu.querySelector("[data-action='color']")?.addEventListener("click", () => cycleMarkerColor(id)); menu.querySelector("[data-action='delete']")?.addEventListener("click", () => deleteMarker(id)); menu.style.left = `${Math.min(window.innerWidth - 230, Math.max(8, x))}px`; menu.style.top = `${Math.min(window.innerHeight - 220, Math.max(8, y))}px`; menu.hidden = false; }
  function ensureModal() { let modal = document.getElementById(MODAL_ID); if (modal) return modal; modal = document.createElement("div"); modal.id = MODAL_ID; modal.className = "orgavox-markers-modal"; modal.hidden = true; modal.innerHTML = `<section class="orgavox-markers-dialog" role="dialog" aria-modal="true" aria-labelledby="markersTitle"><div class="popover-head"><div><span class="eyebrow">Timeline map</span><h3 id="markersTitle">Markers</h3></div><button class="icon-button" data-marker-close type="button">×</button></div><div class="orgavox-marker-help">Rename, retime, recolor, jump, or delete normal markers here. Beat markers are display-only in this phase.</div><div class="orgavox-markers-list" data-markers-list></div><div class="button-row end"><button class="tool-button" data-marker-close type="button">Close</button><button class="tool-button primary" data-marker-add type="button">Add Marker</button></div></section>`; document.body.appendChild(modal); modal.querySelectorAll("[data-marker-close]").forEach((button) => button.addEventListener("click", () => { modal.hidden = true; })); modal.querySelector("[data-marker-add]")?.addEventListener("click", addMarkerAtPlayhead); modal.addEventListener("click", (event) => { if (event.target === modal) modal.hidden = true; }); modal.addEventListener("keydown", (event) => { if (event.key === "Escape") modal.hidden = true; }); return modal; }
  ...
  function renderList() {
    ...
    sorted.forEach((marker) => {
      ...
      row.dataset.markerRow = marker.id;
      row.innerHTML = `<input data-marker-label value="${esc(marker.label)}" aria-label="Marker name"><input data-marker-time value="${formatTime(marker.time)}" aria-label="Marker time"><select data-marker-color aria-label="Marker color">${COLORS.map((color) => `<option value="${color}"${color === marker.color ? " selected" : ""}>${color}</option>`).join("")}</select><button class="tool-button" data-marker-save type="button">Save</button><button class="tool-button" data-marker-jump type="button">Jump</button><button class="tool-button danger" data-marker-delete type="button">Delete</button>`;
      row.querySelector("[data-marker-save]")?.addEventListener("click", () => saveMarkerRow(row));
      row.querySelector("[data-marker-jump]")?.addEventListener("click", () => jumpToMarker(marker.id));
      row.querySelector("[data-marker-delete]")?.addEventListener("click", (event) => { event.preventDefault(); event.stopPropagation(); deleteMarker(marker.id); });
      row.querySelectorAll("input,select").forEach((input) => input.addEventListener("keydown", (event) => { if (event.key === "Enter") saveMarkerRow(row); }));
