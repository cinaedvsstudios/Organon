...
(async () => {
  ...
  function installVisibleUiOwner() {
    if (window.__orgavoxVisibleUiOwner109) return;
    window.__orgavoxVisibleUiOwner109 = true;

    const STYLE_ID = "orgavox-visible-ui-owner-v109";
    ...
    function installStyles() {
      ...
      style.id = STYLE_ID;
      style.textContent = `
        :root{--topbar-h:136px!important;--controls-h:0!important;--orgavox-sidebar-w:288px!important;--lane-h:112px!important}
        ...
        .orgavox-top-row,.orgavox-mix-row{display:flex!important;align-items:center!important;gap:8px!important;min-width:0!important;width:100%!important}.orgavox-top-row{height:42px!important;flex:0 0 42px!important;flex-wrap:nowrap!important}.orgavox-mix-row{height:42px!important;flex:0 0 42px!important;flex-wrap:nowrap!important;padding-left:300px!important}.orgavox-group{display:inline-flex!important;align-items:center!important;gap:8px!important;flex-wrap:nowrap!important;min-width:0!important}.orgavox-spacer{flex:1 1 auto!important}.orgavox-divider{width:1px!important;align-self:stretch!important;min-height:32px!important;max-height:44px!important;margin:0 6px!important;background:linear-gradient(180deg,transparent,rgba(224,163,96,.48),transparent)!important}.orgavox-divider+.orgavox-divider{display:none!important}
        ...
        @media(max-width:1380px){:root{--topbar-h:188px!important;--orgavox-sidebar-w:270px!important}.orgavox-top-row{flex-wrap:wrap!important;height:auto!important}.orgavox-mix-row{padding-left:270px!important;flex-wrap:wrap!important;height:auto!important}}
      `;
    ...
    function group(name, nodes) { const el = document.createElement("span"); el.className = `orgavox-group orgavox-${name}-group`; appendOrdered(el, nodes); return el; }
    function div() { const el = document.createElement("span"); el.className = "orgavox-divider"; return el; }
    ...
    function sendToStart() { const clips = selectedClips(); if (!clips.length) return showToast("Select a clip first."); const earliest = Math.min(...clips.map((clip) => Number(clip.start) || 0)); clips.forEach((clip) => { clip.start = Math.max(0, (Number(clip.start) || 0) - earliest); }); renderTimeline(); syncSelectedControls(); window.orgavoxRecordHistory?.(); }
    function openAnalyze() { const node = document.getElementById("analysisBtn"); node?.click?.(); }
    ...
    function ensureMasterControl() {
      ...
      state.globalVolume = Math.max(0, Math.min(200, Number(state.globalVolume ?? localStorage.getItem("orgavoxGlobalVolume") ?? 100) || 100));
      ui.globalVolumeSlider = control.querySelector("#globalVolumeSlider");
      ui.globalVolumeOut = control.querySelector("#globalVolumeOut");
      ui.globalVolumeSlider.value = state.globalVolume;
    ...
    function ensureSidebar() {
      ...
      let count = document.getElementById("assetCount");
      if (!count) { count = document.createElement("span"); count.id = "assetCount"; count.className = "count-badge"; count.textContent = String(state.assets?.length || 0); ui.assetCount = count; }
      let brand = heading.querySelector(".orgavox-sidebar-brand");
      if (!brand) { brand = document.createElement("div"); brand.className = "orgavox-sidebar-brand"; }
      ...
      if (!zoomWrap) { zoomWrap = document.createElement("div"); zoomWrap.className = "orgavox-sidebar-zoom"; heading.insertAdjacentElement("afterend", zoomWrap); }
      const zoom = ui.zoomSlider?.closest(".range-control");
      if (zoom) {
        zoom.classList.add("zoom-control");
        if (ui.fullscreenBtn) { ui.fullscreenBtn.textContent = "⛶"; zoom.appendChild(ui.fullscreenBtn); }
    ...
    function ensureTrackSkeleton() {
      ...
      ui.lanes.forEach((lane) => {
        ...
        if (!overlay.textContent || overlay.textContent.includes("Track")) overlay.textContent = "VOL 100%";
      });
    }
    ...
    function patchRuntime() {
      if (window.__orgavoxVisibleRuntime109) return;
      window.__orgavoxVisibleRuntime109 = true;
