...
(async () => {
  ...
  function installVisibleUiOwner() {
    if (window.__orgavoxVisibleUiOwner110) return;
    window.__orgavoxVisibleUiOwner110 = true;

    const STYLE_ID = "orgavox-visible-ui-owner-v110";
    ...
    function installStyles() {
      ...
      style.id = STYLE_ID;
      style.textContent = `
        :root{--topbar-h:136px!important;--controls-h:0!important;--orgavox-sidebar-w:250px!important;--orgavox-track-label-w:122px!important;--lane-h:112px!important}
    ...
    function group(name, children) {
      ...
      node.className = `orgavox-group orgavox-${name}-group`;
    ...
    function appendOrdered(parent, children) { children.filter(Boolean).forEach((node) => { if (node.parentElement !== parent || parent.lastElementChild !== node) parent.appendChild(node); }); }
    function getSelectedClip() { return typeof selectedClip === "function" ? selectedClip() : state.clips.find((clip) => clip.id === state.selectedClipId) || null; }
    ...
    function copyClip() { const clips = selectedClips(); if (!clips.length) return toast("Select a clip to copy."); const earliest = Math.min(...clips.map((clip) => Number(clip.start) || 0)); clipClipboard = clips.map((clip) => ({ ...clip, __relativeStart: (Number(clip.start) || 0) - earliest })); toast("Clip copied."); }
    function cutClip() { const clips = selectedClips(); if (!clips.length) return toast("Select a clip to cut."); copyClip(); const ids = new Set(clips.map((clip) => clip.id)); stopPlayback?.(); state.clips = state.clips.filter((clip) => !ids.has(clip.id)); state.selectedClipId = null; state.selectedClipIds = []; syncSelectedControls(); renderTimeline(); window.orgavoxRecordHistory?.(); toast("Clip cut."); }
    function pasteClip() { if (!clipClipboard.length) return toast("No copied clip to paste."); const base = Math.max(0, Number(state.playhead) || 0); const pasted = clipClipboard.map((clip) => ({ ...clip, id: makeId("clip"), start: base + Math.max(0, Number(clip.__relativeStart) || 0), cacheVersion: 0 })); pasted.forEach((clip) => delete clip.__relativeStart); state.clips.push(...pasted); state.selectedClipId = pasted[pasted.length - 1]?.id || null; state.selectedClipIds = pasted.map((clip) => clip.id); renderTimeline(); syncSelectedControls(); window.orgavoxRecordHistory?.(); toast("Clip pasted at the playhead."); }
    function clearAll() { if (!state.clips.length) return toast("Timeline is already clear."); if (window.confirm && !confirm("Clear all clips from the timeline?")) return; stopPlayback?.(); state.clips = []; state.selectedClipId = null; state.selectedClipIds = []; syncSelectedControls(); renderTimeline(); window.orgavoxRecordHistory?.(); }
    function deleteClips() { const clips = selectedClips(); if (!clips.length) return deleteSelectedClip?.(); const ids = new Set(clips.map((clip) => clip.id)); stopPlayback?.(); state.clips = state.clips.filter((clip) => !ids.has(clip.id)); state.selectedClipId = null; state.selectedClipIds = []; syncSelectedControls(); renderTimeline(); window.orgavoxRecordHistory?.(); }
    function sendToStart() { const clips = selectedClips(); if (!clips.length) return toast("Select a clip to send to start."); const earliest = Math.min(...clips.map((clip) => Math.max(0, Number(clip.start) || 0))); clips.forEach((clip) => { clip.start = clips.length === 1 ? 0 : Math.max(0, (Number(clip.start) || 0) - earliest); }); renderTimeline(); syncSelectedControls(); window.orgavoxRecordHistory?.(); }
    ...
    function ensureSidebar() {
      ...
      if (!panel) return;
      const heading = panel.querySelector(".panel-heading") || document.createElement("div");
      heading.className = "panel-heading";
      heading.innerHTML = `<div class="orgavox-sidebar-brand"><span class="orgavox-sidebar-mark">Φ</span><span class="orgavox-sidebar-title"><strong>ORGAVOX <span class="orgavox-sidebar-version">${VERSION}</span></strong><span>Browser Audio Workstation</span></span></div><span class="count-badge" id="assetCount">${state.assets?.length || 0}</span>`;
    ...
    function buildTopbar() {
      ...
      appendOrdered(effects.menu, [document.getElementById("effectsLibraryBtn"), ui.reverseClipBtn].filter(Boolean));
      ...
      if (volume) volume.classList.add("orgavox-clip-volume-control");
      const echo = ui.echoSlider?.closest(".range-control");
      if (echo) {
        echo.classList.add("orgavox-echo-control");
        let gear = document.getElementById("echoSettingsBtn");
        if (!gear) { gear = document.createElement("button"); gear.id = "echoSettingsBtn"; gear.type = "button"; }
        gear.className = "icon-button echo-settings-btn"; gear.textContent = "⚙";
        echo.appendChild(gear);
      }
    ...
    function patchRuntime() {
      if (window.__orgavoxVisibleRuntime110) return;
      window.__orgavoxVisibleRuntime110 = true;
      ...
      if (typeof syncSelectedControls === "function") { const old = syncSelectedControls; syncSelectedControls = function orgavoxUiOwnerSyncSelectedControls() { const result = old.apply(this, arguments); const hasClip = Boolean(getSelectedClip()); [ui.deleteBtn, ui.downloadClipBtn, ui.reverseClipBtn, ui.bounceBtn, ui.scissorsBtn, ui.nudgeLeftBtn, ui.nudgeRightBtn, ui.alignPlayheadBtn, document.getElementById("analysisBtn")].filter(Boolean).forEach((btn) => { btn.disabled = !hasClip; }); return result; }; }
