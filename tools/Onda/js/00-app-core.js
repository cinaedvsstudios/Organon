// --- CORE DUAL AUDIO ENGINE SETUP ---
        ...
        function buildVisualPaletteHtml(currentColor = '#00d8ff') {
            ...
            VISUAL_COLOR_PALETTE_GROUPS.forEach(group => {
                group.forEach(color => swatches.push(`<button type="button" class="vis-palette-swatch" data-color="${color}" style="background:${color}" title="${color}"></button>`));
            });
            ...
            visualizerLayers
                ...
                .forEach((layer, index) => {
                    ...
                    card.innerHTML = `
                        ...
                        <div>
                            <div class="composer-help" style="margin-bottom: 4px;">Colours 1–6. Black = ignored / blank.</div>
                            <div class="visual-color-row">
                                ${safeColors.slice(0, 6).map((color, i) => `<button type="button" class="vis-color-chip vis-layer-color" data-color-index="${i}" data-color="${sanitizeHexColor(color)}" style="background:${sanitizeHexColor(color)}" title="Colour ${i + 1}; black means blank"></button>`).join('')}
                            </div>
                        ...
                        <div class="matrix-extra-settings">
                            ...
                            <div class="visual-check-row matrix-spawn-row">
                                <span style="color:var(--water-spray); font-weight:800; margin-right:4px;">Spawn point</span>
        ...
        function applyLibraryDrawerWidth(width, { persist = false } = {}) {
            ...
            const isMobile = window.matchMedia('(max-width: 768px)').matches;
            if (isMobile) {
                libraryDrawer.style.width = '';
                libraryDrawer.style.left = '';
                libraryDrawer.style.right = '';
                libraryDrawer.style.transform = '';
                return;
            }
            const safeWidth = clampLibraryDrawerWidth(width);
            libraryDrawer.style.width = `${safeWidth}px`;
            libraryDrawer.style.left = `${Math.round((window.innerWidth - safeWidth) / 2)}px`;
            libraryDrawer.style.right = 'auto';
            if (persist) {
        ...
        function applyLibraryDrawerHeight(height, { persist = false } = {}) {
            ...
            if (isMobile) {
                ...
                libraryDrawer.style.bottom = `${bottomOffset}px`;
            }
        ...
        function syncMobileLibraryDrawerBounds() {
            ...
            libraryDrawer.style.bottom = `${bottomEdge}px`;
        }
        ...
        [localAudio, streamAudio].forEach(audioObj => {
            ...
            audioObj.addEventListener('play', (e) => {
                if (e.target === activeAudio) {
                    isPlaying = true;
                    btnPlay.innerHTML = '<span class="playing-spinner">🧿</span>';
        ...
        function scrollCurrentTrackRowsIntoView(reason = 'track-change') {
            ...
            window.OndaActiveTrackScrollTimer = window.setTimeout(() => {
                ...
                window.OndaActiveTrackScrollDebug = {
                    reason,
        ...
        function updateNowPlayingPlaylistPanel() {
            ...
            card.classList.remove('now-playing-song-only', 'now-playing-playlist-only', 'now-playing-basic', 'lyrics-collapsed', 'now-playing-hide-song-info', 'now-playing-hide-playlist');
            ...
            if (pills) {
                ...
                if (playlistBtn) {
                    ...
                    playlistBtn.style.opacity = (!name || !playlists[name]) ? '0.45' : '';
                }
            }
            ...
            if (!name || !playlists[name]) {
                panel.hidden = true;
                panel.innerHTML = '';
                if (lyricsViewer) lyricsViewer.style.maxHeight = '110px';
                return;
            }
            ...
            window.OndaNowPlayingQueueDebug = {
                playlist: name,
                count: ids.length,
        ...
        // --- 5. DRAGGABLE SETTINGS POPUP ENGINE LOGIC ---
        ...
        btnToggleAdvanced.addEventListener('click', () => {
            ...
            btnToggleAdvanced.classList.toggle('active-state', !isHidden);
            
            if (isHidden) {
                advancedControls.style.left = "50%";
                advancedControls.style.top = "30%";
                advancedControls.style.transform = "translate(-50%, -30%)";
            }
        });
        ...
        dragHandle.addEventListener('mousedown', (e) => {
            ...
            dragStartX = e.clientX - advancedControls.offsetLeft;
            dragStartY = e.clientY - advancedControls.offsetTop;
        ...
        function onPopupDragMove(e) {
            ...
            advancedControls.style.left = (e.clientX - dragStartX) + "px";
            advancedControls.style.top = (e.clientY - dragStartY) + "px";
        }
        ...
        dragHandle.addEventListener('touchstart', (e) => {
            ...
            dragStartX = touch.clientX - advancedControls.offsetLeft;
            dragStartY = touch.clientY - advancedControls.offsetTop;
        ...
        function onPopupTouchMove(e) {
            ...
            const touch = e.touches[0];
            advancedControls.style.transform = "none";
            advancedControls.style.left = (touch.clientX - dragStartX) + "px";
            advancedControls.style.top = (touch.clientY - dragStartY) + "px";
        }
        ...
        function renderPlaylistEditTracks(name = editingPlaylistName) {
            ...
            ids.forEach((trackId, index) => {
                ...
                row.innerHTML = `
                    <div class="library-row-main">
                        <div class="library-row-main" style="justify-content:flex-start; min-width:0;">
                            <input type="checkbox" class="playlist-edit-track-check" data-track-id="${escapeHtml(trackId)}" style="margin-right:10px;">
                            <div>
                        ...
                        <div class="library-row-buttons">
                            ...
                            <button type="button" class="btn-pill btn-playlist-edit-remove" data-track-id="${escapeHtml(trackId)}" style="background: var(--brand-red); color:white;">Remove</button>
                        </div>
                    </div>`;
                list.appendChild(row);
            });
        ...
        function renderPlaylistsList() {
            ...
            Object.keys(playlists).sort().forEach(plName => {
                ...
                row.style.background = activePlaylistView === plName ? "rgba(75, 132, 191, 0.15)" : "rgba(0,0,0,0.15)";
                row.style.borderColor = activePlaylistView === plName ? "var(--water-blue)" : "transparent";
                row.dataset.playlistName = plName;
                row.innerHTML = `
                    <div class="playlist-row-content">
                        ${playlistThumbHtml(plName, false)}
                        <div>
                            <div style="color:var(--stone-ochre); font-weight:bold;">▤ ${escapeHtml(plName)}</div>
                            <div style="font-size:0.75rem; color:var(--chiseled-bronze);">${normalisePlaylistToIds(playlists[plName] || []).length} tracks</div>
                        </div>
                    </div>
                    <div class="playlist-row-tools">
                        <button type="button" class="btn-pill btn-view-playlist" style="padding: 2px 10px; font-size:0.7rem; background:${activePlaylistView === plName ? 'var(--brand-red)' : 'transparent'}; color:${activePlaylistView === plName ? 'white' : 'var(--water-spray)'}; border-color:${activePlaylistView === plName ? 'var(--brand-red)' : 'var(--water-blue)'}">
            ...
            panel.innerHTML = `
                ...
                <div class="playlist-detail-track-list" id="playlist-detail-track-list">
                    ${ids.length ? ids.map((id, index) => {
                        ...
                        return `<div class="library-result-row playlist-detail-track-row${isCurrent ? ' active-track now-playing-current-track' : ''}" data-track-id="${escapeHtml(id)}" style="margin-bottom:8px;">
                            <div class="history-list-number">${index + 1}</div>
                            <div class="onda-track-text-wrap">
        ...
        safeBind('btn-playlist', 'click', () => {
            ...
            Object.keys(playlists).forEach(pl => {
                ...
                label.innerHTML = `<input type="checkbox" value="${pl}" ${isAssigned ? 'checked' : ''} style="width: auto;"> ⭐ ${pl}`;
        ...
        function closeAllModals() {
            ...
            document.querySelectorAll('.organon-modal').forEach(m => m.style.display = 'none');
            overlay.style.display = 'none';
        ...
        tagsBar.addEventListener('mousedown', (e) => {
            dragScrollDown = true;
            tagsBar.style.cursor = 'grabbing';
            dragScrollStartX = e.pageX - tagsBar.offsetLeft;
            scrollLeftTags = tagsBar.scrollLeft;
        });
        tagsBar.addEventListener('mouseleave', () => { dragScrollDown = false; tagsBar.style.cursor = 'grab'; });
        tagsBar.addEventListener('mouseup', () => { dragScrollDown = false; tagsBar.style.cursor = 'grab'; });
