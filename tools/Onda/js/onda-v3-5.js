/* Onda v3.7 mobile layout, marquee, repeat, shuffle, and background-media corrections. */
(function () {
    'use strict';
    ...
    if (window.OndaV37?.initialized) return;
    ...
    function switchToTrack(index) {
        ...
        try {
            ...
            console.error('Onda v3.7 could not switch tracks:', error);
            return false;
        }
    }
    ...
    function restartCurrentTrack(audioObject) {
        ...
        try {
            ...
            console.error('Onda v3.7 could not repeat the current track:', error);
        }
    }
    ...
    function installMobileScrollGuards() {
        ...
        try {
            scrollCurrentTrackRowsIntoView = function ondaV37TrackScroll(reason = 'track-change') {
                if (!isMobileMode()) {
            ...
            console.warn('Onda v3.7 could not install the mobile track-scroll guard:', error);
        }

        try { originalMetaEdit = triggerMetaEdit; } catch (error) {}
        try {
            triggerMetaEdit = function ondaV37MetaEdit() {
                if (!isMobileMode()) {
            ...
            console.warn('Onda v3.7 could not install the mobile metadata-scroll guard:', error);
        }
    }
